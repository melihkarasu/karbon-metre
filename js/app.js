...
// Popüler Siteler ve Standart Veri Dağılımı (Önbellek & Benchmark)
const KNOWN_BENCHMARKS = {
  'github.com': { bytes: 575488, isGreen: true, hostingProvider: 'Microsoft Azure (Yenilenebilir)', durationMs: 140 },
  'google.com': { bytes: 245760, isGreen: true, hostingProvider: 'Google Cloud Platform (%100 Yeşil)', durationMs: 95 },
  'wikipedia.org': { bytes: 327680, isGreen: true, hostingProvider: 'Wikimedia Foundation Servers', durationMs: 120 },
  'apple.com': { bytes: 819200, isGreen: true, hostingProvider: 'Apple Green Energy Infrastructure', durationMs: 180 },
  'turkiye.gov.tr': { bytes: 614400, isGreen: false, hostingProvider: 'Türksat A.Ş. Ulusal Veri Merkezi', durationMs: 110 }
};

// 1. Karbon Analizi Çalıştır
async function runCarbonAnalysis(customUrl = null) {
  let url = (customUrl || document.getElementById('input-site-url').value).trim();
  if (!url) return;
  ...
  try {
    ...
    // The Green Web Foundation Açık API Sorgusu (CORS Açık & Ücretsiz)
    let isGreen = false;
    ...
    try {
      ...
      console.warn('The Green Web Foundation API atlandı:', e);
    }

    // Benchmark veya Tahmini Sayfa Ağırlığı Hesaplama (SWD Modeli)
    let bytes = 520000;
    let durationMs = 150;
    let encoding = 'gzip';

    if (KNOWN_BENCHMARKS[hostname]) {
      const b = KNOWN_BENCHMARKS[hostname];
      bytes = b.bytes;
      ...
      // Rastgele gerçekçi varyasyon (SWD Web medyanı ~500KB - 1.2MB)
      const hash = hostname.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
      bytes = 380000 + (hash % 600000);
      durationMs = 90 + (hash % 160);
    }

    // Sustainable Web Design (SWD v4) Emisyon Formülü:
    // E = Veri Transferi (GB) * 0.81 kWh/GB * Karbon Yoğunluğu (gCO2/kWh)
    const gb = bytes / (1024 * 1024 * 1024);
    const carbonIntensity = isGreen ? 50 : 442; // Yeşil: 50 gCO2/kWh, Standart: 442 gCO2/kWh
    const co2Grams = Math.max(0.02, gb * 0.81 * carbonIntensity);

    // Eko Harf Notu ve Yüzdelik Sıralama
    let grade = 'B';
    let cleanerThan = 75;
...
function quickAnalyze(url) {
  const input = document.getElementById('input-site-url');
...
function renderResults(data) {
  ...
  if (data.isGreen) {
    hostBadge.className = 'px-3 py-1.5 rounded-xl border border-emerald-500/40 bg-emerald-500/20 text-emerald-700 flex items-center gap-2 text-xs font-bold shrink-0';
    hostBadge.innerHTML = `<span>🌿</span> <span>Yeşil Hosting (${data.hostingProvider || 'Yenilenebilir Enerji'})</span>`;
  } else {
  ...
  // Dinamik SVG Rozet Üretimi (%100 İstemci Taraflı Data URI)
...
function generateBadgePreview(data) {
  ...
  container.innerHTML = `<img src="${svgDataUri}" alt="Eko-Skor Rozeti" class="shadow-lg rounded-md">`;

  const htmlCode = `<a href="https://melihkarasu.github.io/karbon-metre/" target="_blank" rel="noopener"><img src="${svgDataUri}" alt="Web Karbon Ayak İzi" /></a>`;
  const mdCode = `[![Web Karbon Ayak İzi](${svgDataUri})](https://melihkarasu.github.io/karbon-metre/)`;
    ...
    <div class="p-4 rounded-2xl bg-white border border-mistral-hairline hover:border-lime-500/40 transition cursor-pointer flex items-center justify-between" onclick="quickAnalyze('https://${item.cleanHost}')">
      <div class="min-w-0 pr-2">
        ...
        <div class="flex items-center gap-1.5 text-[10px] text-mistral-slate mt-0.5">
          <span class="font-mono text-lime-600 font-bold">${item.co2Grams.toFixed(2)}g CO2</span>
          <span>•</span>
          <span>${item.date}</span>
        </div>
      </div>
